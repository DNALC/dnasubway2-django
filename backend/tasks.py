...
@shared_task
def submit_rarefaction_job_task(job_id, rooted_tree_qza_path, trim_table_qza_path, metadata_path, minDepth, maxDepth):
    job = Job.objects.get(id=job_id)
    project = job.project
    user = job.user
    ...
    job_params = {
        ...
        "parameterSet": {
            "envVariables": [
                ...
                {"key": "jobName", "value": "trim-ar"},
                {"key": "metadata", "value": "metadata.tsv"},
                {"key": "minDepth", "value": str(minDepth)},
                {"key": "maxDepth", "value": str(maxDepth)},
            ]
        }
    }
...
@shared_task
def submit_ancom_job_task(job_id, trim_table_qza_path, taxonomy_qza_path, metadata_path, category, formula, taxalevel):
    job = Job.objects.get(id=job_id)
    project = job.project
    user = job.user
    ...
    job_params = {
        "name": "ancom",
        "appId": settings.QIIME2_ANCOM_APP_TAPIS_ID,
        "appVersion": settings.QIIME2_ANCOM_APP_VERSION,
        "fileInputs": fileInputs,
        "parameterSet": {
            "envVariables": [
                {"key": "DEBUG", "value": "1"},
